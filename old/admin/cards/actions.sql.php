...
// Allowed: super admins, judges
\App\Legacy\Authorization::allow([1, 2]);

$input = input();

// ERROR: No action passed
if (!$input->has('action', 'POST')) {
	alert('You did not pass any action', 'warning');
	redirect_old();
}

$action = $input->post('action');
if     ($action === 'create') require __DIR__ . '/crud/create.php';
elseif ($action === 'update') require __DIR__ . '/crud/update.php';
elseif ($action === 'delete') require __DIR__ . '/crud/delete.php';

// EDIT CARD --------------------------------------------------------------
elseif ($input->post('action') === 'edit' && $input->has('id')) {
	...
	// Passed a backside
	if ($input->has('backside')) {
		$bs = $input->post('backside');
		if     ($bs === 'sh') [ $backside, $suffix ] = [ 2, 'sh' ];
		elseif ($bs === 'jj') [ $backside, $suffix ] = [ 3, 'jj' ];
		elseif ($bs === 'in') [ $backside, $suffix ] = [ 4, 'in' ];
	}
	
	// J-ruler (backside = 1)
	elseif ($input->post('type') === 'J-Ruler') {
		[ $backside, $suffix ] = [ 1, 'j' ];
	}

	// Alternate art
	elseif ($input->post('narp') === 1) {
		[ $backside, $suffix ] = [ 0, 'sr' ];
	}
	...
	$card_link = "<strong><a href='{$card_url}' target=_blank>".$_POST['name']."</a></strong>";

	alert("Card {$card_link} successfully edited.", 'info');
...
elseif ($_POST['action'] === 'create') {
	...
	if (!empty($existingCard)) {
		...
		redirect_old('admin/cards', ['form_action' => 'create']);
	}
	...
	// Notify the user
	alert("New card {$card_link} successfully added.", 'success');
